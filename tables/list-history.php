...
EOS;
		...
		//Status
		$where[] = $wpdb->prepare('(t.status IN (%s, %s, %s, %s, %s) OR (t.status = %s AND t.method IN (%s, %s, %s, %s, %s, %s)) OR ( (t.status = %s) AND (pm2.meta_value IS NOT NULL) AND (TO_DAYS(NOW()) <= TO_DAYS(pm2.meta_value))) )',
					...
					LWP_Payment_Methods::TRANSFER, LWP_Payment_Methods::SOFTBANK_PAYEASY, LWP_Payment_Methods::SOFTBANK_WEB_CVS, LWP_Payment_Methods::GMO_WEB_CVS, LWP_Payment_Methods::GMO_PAYEASY, LWP_Payment_Methods::NTT_CVS,
